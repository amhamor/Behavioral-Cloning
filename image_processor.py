...
def crop_image(image, x_pixel_removal_count=60):
#The following x and y variables are in reference to scipy.ndimage (i.e. the origin is in the top left corner of the image):
	...
	cropped_image = np.zeros(shape=(x_pixel_count-x_pixel_removal_count, y_pixel_count))
	...
	for cropped_x_pixel_index in range(cropped_x_pixel_starting_index, cropped_x_pixel_ending_index-1, -1):
		...
		cropped_image[cropped_x_pixel_index:, y_pixel_index:y_pixel_ending_index] = image[cropped_x_pixel_index+x_pixel_removal_count:, y_pixel_index:y_pixel_ending_index]

	#Crop a rectangle in the middle of the image.
	cropped_image[:, y_pixel_ending_index:235] = image[x_pixel_removal_count:, y_pixel_ending_index:235]
	...
	for cropped_x_pixel_index in range(cropped_x_pixel_starting_index, cropped_x_pixel_ending_index+1):
		...
		cropped_image[cropped_x_pixel_index:, y_pixel_starting_index:y_pixel_index] = image[cropped_x_pixel_index+x_pixel_removal_count:, y_pixel_starting_index:y_pixel_index]
